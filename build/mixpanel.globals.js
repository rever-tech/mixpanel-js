(function () {
    ...
    _.safewrap = function(f) {
        return function() {
            try {
                ...
                console$1.critical('Implementation error. Please contact fap@rever.com.');
            }
        };
    };
    ...
    */
    ...
    MixpanelPersistence.prototype._add_to_people_queue = function(queue, data) {
        ...
        console$1.log('FAP PEOPLE REQUEST (QUEUED, PENDING IDENTIFY):');
        console$1.log(data);
    ...
    var create_mplib = function(token, config, name) {
        ...
        console$1.log(instance.get_config('autotrack'));
        console$1.log(instance)
        if (instance.get_config('autotrack')) {
            ...
            if (!autotrack.enabledForProject(instance.get_config('token'), num_buckets, num_enabled_buckets)) {
                ...
                console$1.log('Enable autotrack!');
            }
            ...
            console$1.log('autotrack disabled');
        }
    ...
    MixpanelLib.prototype.init = function (token, config, name) {
        ...
        if (name === PRIMARY_INSTANCE_NAME) {
            console$1.error('You must initialize the main fap object right after you include the FAP js snippet');
            return;
        }
    ...
    MixpanelLib.prototype.track = function(event_name, properties, callback) {
        ...
        if (_.isUndefined(event_name)) {
            console$1.error('No event name provided to fap.track');
            return;
        }
    ...
    MixpanelLib.prototype.time_event = function(event_name) {
        if (_.isUndefined(event_name)) {
            console$1.error('No event name provided to fap.time_event');
            return;
        }
    ...
    MixpanelLib.prototype._check_and_handle_notifications = function(distinct_id) {
        ...
        console$1.log('FAP NOTIFICATION CHECK');
            ...
            _.each(prop, function(v, k) {
                if (!this._is_reserved_property(k)) {
                    if (isNaN(parseFloat(v))) {
                        console$1.error('Invalid increment value passed to fap.people.increment - must be a number');
                        return;
                    } else {
                        $add[k] = v;
                    }
    ...
    MixpanelPeople.prototype.track_charge = function(amount, properties, callback) {
        if (!_.isNumber(amount)) {
            ...
            if (isNaN(amount)) {
                console$1.error('Invalid value passed to fap.people.track_charge - must be a number');
                return;
            }
        }
    ...
    MixpanelPeople.prototype.delete_user = function() {
        if (!this._identify_called()) {
            console$1.error('fap.people.delete_user() requires you to call identify() first');
            return;
        }
    ...
    MixpanelPeople.prototype._send_request = function(data, callback) {
        ...
        console$1.log('FAP PEOPLE REQUEST:');
        console$1.log(truncated_data);
    ...
    function init_from_snippet() {
        ...
        if (_.isUndefined(mixpanel_master)) {
            ...
            console$1.critical('"fap" object not initialized. Ensure you are using the latest version of the FAP JS Library along with the snippet we provide.');
            return;
        }
        if (mixpanel_master['__loaded'] || (mixpanel_master['config'] && mixpanel_master['persistence'])) {
            // lib has already been loaded at least once; we don't want to override the global object this time so bomb early
            console$1.error('FAP library has already been downloaded at least once.');
            return;
        }
        var snippet_version = mixpanel_master['__SV'] || 0;
        if (snippet_version < 1.1) {
            // mixpanel wasn't initialized properly, report error and quit
            console$1.critical('Version mismatch; please ensure you\'re using the latest version of the FAP code snippet.');
            return;
        }
